...
const ItemsManager = () => {
	...
	return (
		<PageGAC>
			<ConfirmModal isOpen={openModal} message={'¿Estás seguro de que quieres eliminar este objeto? Esta acción es irreversible.'} onConfirm={deleteItem} onCancel={() => setOpenModal(false)} />
			...
			<div className="flex flex-row justify-justify p-5 w-full">
				{createItem ? (
					...
					<>
						{editItem ? (
							selectedItem && <ItemForm itemData={selectedItem} handleSubmit={onUpdateItem} />
						) : (
							<>
								<ItemsList handleShow={handleShow} handleEdit={handleEdit} handleDelete={handleDelete} />
								{selectedItem && (
									<InfoGAC>
