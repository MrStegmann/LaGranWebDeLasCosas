...
const GearExtension = ({ charGear, charInventory, handleSetGear, handleSetInventory }) => {
	...
	useEffect(() => {
		...
		const weaponItems = items.filter((item) => item.code === 'weapon' || item.code === 'shield');
		weaponItems.forEach((item) => {
			weaponSetters[item.slot]((before) => [...before, item]);
		});

		const otherItems = items.filter((item) => item.code !== 'weapon' && item.code !== 'armor' && item.code !== 'shield');
		otherItems.forEach((item) => setOthersList((before) => [...before, item]));
	}, []);

	useEffect(() => {
		handleSetGear(new Gear(head, shoulders, chest, hands, legs, feet, necks, rings, trinkets, firstHand, secondHand));
	}, [head, shoulders, chest, hands, legs, feet, necks, rings, trinkets, firstHand, secondHand]);
	useEffect(() => {
	...
	const setGearSlot = (e) => {
		...
		const found = lists[e.target.id].find((item) => item._id === e.target.value);
		if (found) {
	...
	const onAddItem = (e) => {
		...
		if (inventory.length >= 10) {
			setAlert({ msg: 'No puedes tener más de 10 objetos en el inventario', type: 'error' });
			return;
		}
	...
	const onRemoveItem = (e) => {
		e.preventDefault();
		setInventory((before) => {
			const temp = [...before.filter((item) => item.inventoryId !== e.target.id)];
			return temp;
		});
	};
	...
	return (
		<section className="p-1 mt-6 relative">
			<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
				{['head', 'shoulders', 'chest', 'hands', 'legs', 'feet', 'firstHand', 'secondHand'].map((slot) => (
					<div key={slot} className="flex flex-col md:flex-row items-center">
						<label htmlFor={slot} className="block mb-1 font-semibold w-1/5">
							{_('es', slot)}
						</label>
						<SelectGAC customClass={'w-1/2'} id={slot} value={values[slot]._id} onChange={setGearSlot}>
			...
			<ItemsSelection title="Inventario" atUsed={'inventoryId'} mainItems={inventory} handleOnAdd={onAddItem} handleOnRemove={onRemoveItem} listItems={othersList} />
		</section>
	);
};
