...
const ProsConsExtension = ({ pros, cons, attributes, handleSpecialsPros }) => {
	...
	return (
		<section className="p-1 mt-6">
			{pros && (
				<>
					<h3 className="text-[#e0cda9] font-semibold text-lg mb-4 border-b border-[#6f553e] pb-1">Pros</h3>
									...
									if (key === 'TALENT:adapt') {
										return (
											<li key={key + 'PROS'}>
												...
												<SelectGAC value={toAdapt} onChange={(e) => setToAdapt(e.target.value)}>
													<option value={''}>Sin elegir</option>
													{talentsToAdapt.map((value) => (
														<option key={value.code + 'PROS'} value={value.code}>
															{_('es', value.code.includes(':') ? value.code.split(':')[1] : value.code)}
														</option>
													))}
												</SelectGAC>
											</li>
									...
									if (key === 'TALENT:improvement') {
										return (
											<li key={key + 'PROS'}>
												...
												<SelectGAC value={toImprove} onChange={(e) => setToImprove(e.target.value)}>
													<option value={''}>Sin elegir</option>
