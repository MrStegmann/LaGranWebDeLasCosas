...
const AttirubteExtension = ({ charAtts, handleSetAttributes }) => {
  ...
  useEffect(() => {
    if (!initialized.current && charAtts.length > 0) {
      setAttributes(
        charAtts.map((at) => Attribute.setAttributeByAttribute(at))
      );
  ...
  useEffect(() => {
    setAttributes(() =>
      atts.map((at) => Attribute.setAttributeByAttribute(at))
    );
  }, []);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 mt-2">
        {attributes.length > 0 &&
          attributes.map((at, index) => (
            <div
              key={at.code}
              className={`my-2 px-2 ${[3, 6, attributes.length].includes(index + 1) ? "" : "md:border-r"}`}
            >
              <div className="flex flex-row text-center justify-between items-center border-b pb-2">
                <label
                  htmlFor={at.code + "-input"}
                  className="text-[#dbc59a] text-2xl px-2 font-black"
                >
                  {at.label}
                </label>
              ...
              {at.tp - at.getTalentsTotalPoints() < 0 && (
                <p className="text-red-500 text-center">
                  No tienes puntos de Talento suficiente
                </p>
              )}
              {at.talents.map((tal) => (
                <div
                ...
                >
                  ...
                  <InputGAC
                    ...
                    customClass={"w-1/3"}
                    onChange={(e) =>
                      handleChangeTalValue(e.target.id, e.target.value, at.code)
                    }
                  />
                </div>
              ))}
