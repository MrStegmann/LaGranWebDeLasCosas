...
const ArmorExtension = ({ armor, handleSetArmor }) => {
	...
	useEffect(() => {
		...
		if (armorData.requirements) {
			setRequirements((before) => ({ ...before, ...armorData.requirements[slot] }));
		}
		if (armorData.cons) {
	...
	useEffect(() => {
		...
		if (armorData.requirements) {
			if (reinforcement.includes('-')) {
				setReinforcementRequirements((before) => ({ ...before, ...armorData.requirements }));
			} else {
				setReinforcementRequirements((before) => ({ ...before, ...armorData.requirements[slot] }));
			}
		}
		if (armorData.cons) {
			if (reinforcement.includes('-')) {
				...
				setReinforcementCons((before) => ({ ...before, ...armorData.cons[slot] }));
			}
		}
	}, [reinforcement]);
	...
	useEffect(() => {
		...
		handleSetArmor({ type, slot, pDefense: totalPDefense, sDefense: totalSDefense, durability: totalDurability, reinforcement, requirements: reqs, cons: co });
	}, [
		type,
		slot,
		pDefense,
	...
	const flatRequirements = (requirements, reinforcementRequirements) => {
		...
		if (!reinforcement.includes('-')) {
			if ((reinforcement === 'mail' || reinforcement === 'plate') && (type === 'plate' || type === 'mail')) {
	...
	return (
		<section className="p-4 mt-6">
			<h3 className="text-[#e0cda9] font-semibold text-lg mb-4 border-b border-[#6f553e] pb-1">Detalles de Armadura</h3>

			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div>
					<label htmlFor="type" className="block mb-1 font-semibold tracking-wide text-[#dbc59a]">
						Tipo
					</label>
				...
				<div>
					<label htmlFor="slotArmor" className="block mb-1 font-semibold tracking-wide text-[#dbc59a]">
						Pieza
					</label>
					<SelectGAC id="slotArmor" value={slot} onChange={(e) => setSlot(e.target.value)}>
				...
				<div>
					<label htmlFor="pDefense" className="block mb-1 font-semibold tracking-wide text-[#dbc59a]">
						Reducción Física
					</label>
					<InputGAC id="pDefense" type="number" value={pDefense} min={0} onChange={(e) => setPDefense(e.target.value)} />
					{reinforcement.length > 0 && <p>(+{reinforcementPDefense} Refuerzo)</p>}
				</div>

				<div>
					<label htmlFor="sDefense" className="block mb-1 font-semibold tracking-wide text-[#dbc59a]">
						Reducción Mágica
					</label>
					<InputGAC id="sDefense" type="number" value={sDefense} min={0} onChange={(e) => setSDefense(e.target.value)} />
					{reinforcement.length > 0 && <p>(+{reinforcementSDefense} Refuerzo)</p>}
				</div>

				<div>
					<label htmlFor="durability" className="block mb-1 font-semibold tracking-wide text-[#dbc59a]">
						Durabilidad
					</label>
					<InputGAC id="durability" type="number" value={durability} min={1} onChange={(e) => setDurability(e.target.value)} />
					{reinforcement.length > 0 && <p>(+{reinforcementDurability} Refuerzo)</p>}
				</div>

				<div>
					<label htmlFor="reinforcement" className="block mb-1 font-semibold tracking-wide text-[#dbc59a]">
						Refuerzo
					</label>
					<SelectGAC id="reinforcement" value={reinforcement} onChange={(e) => setReinforcement(e.target.value)}>
						...
						{type !== 'leather' && <option value="leather">(Armadura) Cuero</option>}
						{type !== 'mail' && <option value="mail">(Armadura) Malla</option>}
						{type !== 'plate' && <option value="plate">(Armadura) Placa</option>}
					</SelectGAC>
					<p className="italic text-sm">Elegir un refuerzo de tipo Armadura conlleva penalizadores</p>
				</div>
				...
				<div className="my-5">
					...
					{Object.entries(reinforcementRequirements).length ? (
						<FieldSetGAC title={'De Refuerzo'}>
							<ul className="grid grid-cols-1 md:grid-cols-2 gap-4 list-disc px-5">
								{Object.entries(reinforcementRequirements).map(([key, value]) => (
									<li key={key}>
										{key}: {value}
									</li>
								))}
				...
				<div>
					<h3 className="text-[#e0cda9] font-semibold text-lg mb-4 border-b border-[#6f553e] pb-1">Contras</h3>
