	...
	return (
		<section aria-label="Lista de NPCs" className="w-full md:w-1/2 bg-[#3c291f]/80 rounded-2xl p-6 shadow-lg shadow-black/50 border border-[#5a3b2e] text-[#f0d9b5] flex flex-col items-center">
			...
			<div className="w-full flex flex-row justify-between">
				<InputGAC id="searchInput" type="search" placeholder="Buscar NPC por nombre..." value={search} onChange={(e) => setSearch(e.target.value)} />
				<InputGAC id="levelFilter" type="search" placeholder="Buscar NPC por nivel..." value={levelFilter} onChange={(e) => setLevelFilter(e.target.value)} />
			...
			{/* NPC List */}
			<ul id="npcList" className="w-full mt-5 md:w-3/4 space-y-3 max-h-[420px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-[#7b5a3c] scrollbar-track-[#2f1f17]">
				{filteredNPCs.length > 0 ? (
					filteredNPCs.map((npc) => (
						<LiGAC key={npc._id}>
							...
							<div>
								<SUDButtonsGAC onShow={() => handleShow(npc)} onUpdate={() => handleEdit(npc)} onDelete={() => handleDelete(npc)} />
							</div>
						</LiGAC>
					))
				) : (
					<p className="text-sm text-center text-yellow-200 italic">No se encontraron NPCs</p>
				)}
			</ul>
		</section>
	);
