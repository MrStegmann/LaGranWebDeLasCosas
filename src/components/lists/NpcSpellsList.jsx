...
const NpcSpellsList = ({ nSpells, handleRemoveSpell }) => {
  ...
  const filteredSpells = nSpells.filter((spell) =>
  ...
  return (
    ...
    >
      ...
      <ul
        id="npcSpellList"
        className="mt-5 w-full md:w-3/4 space-y-3 max-h-[420px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-[#7b5a3c] scrollbar-track-[#2f1f17]"
      >
        {filteredSpells.length > 0 ? (
          filteredSpells.map((spell) => (
            ...
            >
              ...
              <button
                onClick={(e) => handleRemoveSpell(e, spell)}
          ...
          <p className="text-sm text-center text-yellow-200 italic">
            No se encontraron hechizos disponibles
          </p>
        )}
      </ul>
    </div>
