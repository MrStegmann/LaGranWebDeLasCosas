...
const NpcInfo = ({ npcData }) => {
	...
	return (
		<div className="bg-[#3c291f]/80 p-6 rounded-2xl shadow-xl shadow-black/60 border-4 border-[#5a3b2e] text-[#f0d9b5] w-full max-w-4xl mx-auto font-serif tracking-wide">
			<h2 className="text-3xl font-extrabold text-center border-b-2 border-[#f0d9b5] pb-2 mb-4">{npcState.name}</h2>
			...
			{/* Atributos */}
			<h3 className="text-2xl font-bold border-b border-[#f0d9b5] pb-1 mb-2 mt-6">Atributos</h3>
			...
			<div className="grid grid-cols-2 gap-6 mt-6">
				<div>
					<h3 className="text-xl font-semibold border-b border-[#f0d9b5] mb-2">Daño</h3>
					<p>Físico: {npcState.pDamage}</p>
					<p>Mágico: {npcState.sDamage}</p>
				</div>
				<div>
					<h3 className="text-xl font-semibold border-b border-[#f0d9b5] mb-2">Defensa</h3>
			...
			{npcState.spells?.length > 0 && (
				<div className="mt-6">
					<h3 className="text-2xl font-bold border-b border-[#f0d9b5] pb-1 mb-2">Hechizos</h3>
			...
			{npcState.skills?.length > 0 && (
				<div className="mt-6">
					<h3 className="text-2xl font-bold border-b border-[#f0d9b5] pb-1 mb-2">Habilidades</h3>
					<ul className="space-y-2 list-disc ml-6">
						{npcState.skills.map((skill) => (
							<li key={skill._id} className="border-b border-[#f0d9b5] pb-2">
								<p className="text-lg font-semibold text-yellow-100">
									{skill.name}
									<span className="text-sm font-normal text-gray-300">
										{' ( '}Tipo: {skill.type} | Acciones: {skill.actions} | Turnos: {skill.turns}
										{' )'}
									</span>
								</p>
