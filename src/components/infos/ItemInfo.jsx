    ...
    <FieldSetGAC title={"Estadísticas de Armadura"}>
      ...
      {reinforcement && (
        <>
          ...
          {(reinforcement.type === "mail" || reinforcement.type === "plate") &&
            (type === "plate" || type === "mail") && (
              <p className="italic">
                Esta armadura tiene los requerimientos y penalizadores
                duplicados
              </p>
            )}
          {reinforcement.type === "leather" &&
            (type === "plate" || type === "mail") && (
              <p className="italic">
                Esta armadura tiene los penalizadores duplicados
              </p>
            )}
        </>
      )}
...
const ItemInfo = ({ item }) => {
  if (!item) return null;
  const { name, description, quality, code, usable, wearable, price, max } =
    item; //Atributos básicos;
