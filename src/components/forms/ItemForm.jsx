...
const ItemForm = ({ itemData, handleSubmit }) => {
	...
	const onSubmit = (e) => {
		e.preventDefault();
		if (name.length === 0) {
			setAlert({ msg: 'Debes introducir un nombre para el objeto', type: 'error' });
			return;
		}

		let item = { id, name, description, code, quality, wearable, usable, price, max };
		if (code === 'armor') {
			item = { ...item, ...armor };
		}
		if (code === 'weapon') {
		...
		<FormGAC onSubmit={onSubmit}>
			...
			<FieldSetGAC title={'Información general'}>
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
					...
					<div>
						...
						<InputGAC id="price" type="text" value={price} onChange={(e) => setPrice(e.target.value)} />
					</div>
			...
			<FieldSetGAC title={'Atributos'}>
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
					<div>
						...
						<SelectGAC id="code" value={code} onChange={(e) => setCode(e.target.value)}>
					...
					<div>
						...
						<SelectGAC id="quality" value={quality} onChange={(e) => setQuality(e.target.value)}>
					...
					<div>
						...
						<InputGAC id="max" type="number" value={max} min={1} onChange={(e) => setMax(e.target.value)} />
					</div>
			...
			{['armor', 'weapon', 'shield', 'ammo'].includes(code) && (
				<FieldSetGAC title={'Atributos específicos'}>
					<div className="mt-4">
						{code === 'armor' && <ArmorExtension armor={armor} handleSetArmor={handleSetArmor} />}
						{code === 'weapon' && <WeaponExtension weapon={weapon} handleSetWeapon={handleSetWeapon} />}
						{code === 'shield' && <ShieldExtension shield={shield} handleSetShield={handleSetShield} />}
						{code === 'ammo' && <AmmoExtension ammo={ammo} handleSetAmmo={handleSetAmmo} />}
					</div>
				</FieldSetGAC>
			)}
