...
const PerkForm = ({ title, perkData, handleSubmit }) => {
  ...
  const handleAddLevel = (e) => {
    ...
    setLevels((previor) => [
      ...previor,
      { ...levelAttributes, id: IdGenerator() },
    ]);
  ...
  const onSubmit = (e) => {
    ...
    if (name.length === 0) {
      setAlert({
        msg: "Debes escribir un nombre para este Rasgo",
        type: "error",
      });
      return;
    }
    if (description.length === 0) {
      setAlert({
        msg: "Debes escribir una descripción para este Rasgo",
        type: "error",
      });
      return;
    }
    ...
    <FormGAC title={title} onSubmit={onSubmit}>
        ...
        <div className="flex flex-col">
          <label>
            ...
            <SelectGAC
              id="level"
              name="level"
              value={levelAttributes.level}
              onChange={handleSetLevelAttributes}
            >
          ...
          <input
            ...
            className="mt-2 text-green-500 hover:text-green-400 hover:underline"
          />
        </div>
      ...
      {levels.map((l, i) => (
        <div key={`${l.id}-${i}`}>
          ...
          <button
            className="mt-2 text-red-500 hover:text-red-400 hover:underline"
