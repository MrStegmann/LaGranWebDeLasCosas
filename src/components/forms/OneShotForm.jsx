...
const StageForm = ({ stageName, stage, onChange }) => {
	...
	return (
		<FieldSetGAC title={stageName}>
			<InputGAC type="text" placeholder="Nombre" value={stage.name} onChange={(e) => handleChange('name', e.target.value)} />

			<TextareaGAC placeholder="Descripción" value={stage.description} onChange={(e) => handleChange('description', e.target.value)} />
			<p className={`${stage.description.length > 560 && 'text-red-800'}`}>{stage.description.length}/560</p>

			<InputGAC type="text" placeholder="Objetivo" value={stage.target} onChange={(e) => handleChange('target', e.target.value)} />
			...
			{stage.cluesHooks.map((clue, index) => (
				<div key={clue + index} className="flex gap-2 mb-2">
					<InputGAC type="text" value={clue} onChange={(e) => handleCluesChange(index, e.target.value)} />

					<SUDButtonsGAC onDelete={() => removeClue(index)} />
				</div>
			))}
			<button onClick={addClue} className="text-yellow-500 hover:text-yellow-400 hover:cursor-pointer hover:underline">
...
const OneShotForm = ({ title, oneshotData, handleSubmit }) => {
	...
	return (
		<FormGAC title={title} onSubmit={onSubmit}>
			<InputGAC type="text" placeholder="Título del Oneshot" value={oneshot.title} onChange={(e) => setOneshot({ ...oneshot, titulo: e.target.value })} />

			<TextareaGAC placeholder="Etiquetas" value={oneshot.flags} onChange={(e) => setOneshot({ ...oneshot, resumen: e.target.value })} />

			<StageForm stageName="Intro" stage={oneshot.intro} onChange={(stage) => handleStageChange('intro', stage)} />
			<StageForm stageName="Nudo" stage={oneshot.knot} onChange={(stage) => handleStageChange('knot', stage)} />
			<StageForm stageName="Desenlace" stage={oneshot.conclusion} onChange={(stage) => handleStageChange('conclusion', stage)} />
