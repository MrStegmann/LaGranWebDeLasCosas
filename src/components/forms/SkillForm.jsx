...
const SkillForm = ({ skillData, handleSubmit }) => {
	...
	return (
		<form onSubmit={onSubmit} className="space-y-6 bg-[#3c291f]/80 p-6 rounded-2xl shadow-lg shadow-black/50 border-2 border-[#5a3b2e] text-[#f0d9b5] w-1/2">
			<div>
			...
			<div className="grid grid-cols-2 gap-4">
				<div>
					<label htmlFor="actions" className="block mb-1 font-semibold tracking-wide">
						Acciones
					</label>
					<InputGAC id="actions" type="number" value={actions} onChange={(e) => setActions(Number(e.target.value))} />
				</div>

				<div>
					<label htmlFor="turns" className="block mb-1 font-semibold tracking-wide">
						Turnos
					</label>
					<InputGAC id="turns" type="number" value={turns} onChange={(e) => setTurns(Number(e.target.value))} />
				</div>
			</div>
