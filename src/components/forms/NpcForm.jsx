...
const NpcForm = ({ npcData, handleSubmit }) => {
	...
	useEffect(() => {
		setApRemaining(ap);
		const totalUsed = strength + dexterity + constitution + intelligence + willpower + wisdom + charisma;

		setApRemaining(ap - totalUsed);
	}, [strength, dexterity, constitution, intelligence, willpower, wisdom, charisma, ap]);

	useEffect(() => {
		const totalUsed = spells.reduce((prev, actualy) => prev + actualy.level, 0) + skills.length;
		setSpRemaining(sp - totalUsed);
	}, [sp, skills, spells]);

	useEffect(() => {
		const { hp, sp, ap } = Npc.stimateStats(level, category);
		const { pDefense, sDefense, pDamage, sDamage } = Npc.stimateDamageDefense({ strength, intelligence, willpower, constitution }, weapon, category);
	...
	}, [level, category, strength, intelligence, constitution, willpower, weapon]);
	...
	const onSubmit = (e) => {
		...
		if (apRemaining < 0) {
			setAlert({ msg: 'No tienes puntos de atributo suficientes', type: 'error' });
			return;
		}

		if (spRemaining < 0) {
			setAlert({ msg: 'No tienes puntos de hechizo/habilidad suficientes', type: 'error' });
			return;
		}
		handleSubmit(Npc.fromRaw({ id, name, level, category, strength, dexterity, intelligence, willpower, constitution, wisdom, charisma, weapon, spells, skills }));

		onReset();
	};

	const stats = { strength, dexterity, intelligence, willpower, constitution, wisdom, charisma };
		...
		<FormGAC onSubmit={onSubmit}>
			<FieldSetGAC title={'Información general'}>
				...
				<div className="flex gap-6">
					<div className="flex-1">
						<label htmlFor="level" className="block mb-1 font-semibold tracking-wide">
							Nivel
						</label>
						<InputGAC id="level" type="number" min={1} max={maxLevel} value={level} onChange={handleLevelChange} />
					</div>

					<div className="flex-1">
						<label htmlFor="category" className="block mb-1 font-semibold tracking-wide">
							Categoría
						</label>
			...
			<FieldSetGAC title={'Atributos'}>
				<div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-4">
					{['strength', 'dexterity', 'intelligence', 'willpower', 'constitution', 'wisdom', 'charisma'].map((stat) => (
						<div key={stat} className="flex flex-col">
							<label htmlFor={stat} className="block mb-1 font-semibold tracking-wide text-[#dbc59a]">
								{stat.toUpperCase()}
							</label>
							<InputGAC id={stat} type="number" min={0} value={stats[stat]} onChange={(e) => handleStatChange(stat, e.target.value)} />
						</div>
					))}
				</div>

				{apRemaining < 0 && <p className="text-red-500 font-semibold">No tienes puntos de Atributo suficiente</p>}
			</FieldSetGAC>

			<FieldSetGAC title={`Daño y Defensa`}>
				<div className="mt-6">
					<label htmlFor="weapon" className="block mb-1 font-semibold tracking-wide">
						Arma
					</label>
					<SelectGAC id="weapon" value={weapon} onChange={(e) => setWeapon(e.target.value)}>
			...
			{spRemaining < 0 && <p className="text-red-500 font-semibold mt-2">No tienes puntos de Habilidad/Hechizo suficiente</p>}
